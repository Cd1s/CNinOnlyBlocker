...
if [ -f "$CONFIG_DIR/ipv6_enabled" ]; then
    ...
    # 配置 ip6tables 规则 - 腾讯云特殊配置
    ip6tables -F INPUT
    ip6tables -A INPUT -m state --state ESTABLISHED,RELATED -j ACCEPT
    ip6tables -A INPUT -i lo -j ACCEPT
    
    # 添加腾讯云内部网络放行
    ip6tables -A INPUT -s fe80::/10 -j ACCEPT  # 链路本地地址
    ip6tables -A INPUT -d fe80::/10 -j ACCEPT
    ip6tables -A INPUT -s ::1/128 -j ACCEPT    # 本地环回
    ...
    # 允许ICMPv6类型 (关键步骤 - 不要阻止ICMPv6)
    ip6tables -A INPUT -p ipv6-icmp -j ACCEPT
    
    # 替换默认DROP为记录后拒绝
    ip6tables -A INPUT -m limit --limit 5/min -j LOG --log-prefix "IPv6 DROP: " --log-level 7
